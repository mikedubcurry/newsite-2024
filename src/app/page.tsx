            ...
            <div className='flex md:flex-row flex-col justify-center items-stretch gap-6 h-full w-full'>
                {cards.map((card, i) => (
                    <Link href={card.href} key={i} className="group w-full md:w-1/3 rounded-xl border border-gray-300 hover:border-yellow-600 flex flex-col justify-center items-center gap-4 p-8 transition-all duration-300 hover:bg-blue-900 hover:scale-[103%]">
                        <h2 className="text-gray-400 text-3xl font-bold group-hover:text-purple-400 transition-colors duration-300 group-hover:underline">{card.title}</h2>
                        <p className="text-xl text-gray-500 group-hover:text-white transition-colors ">{card.description}</p>
                    </Link>
                ))}
            </div>
            <div className="flex flex-col justify-center gap-4">
                <h2 className="text-gray-400 text-3xl font-bold">About</h2>
                <p className='leading-8 mb-2 text-lg text-gray-500'>
